...
local function ApplyDefaults()
  ...
  -- Defaults where false or nil is same as not set
  local defaultsNot = {
...
local color    = HonorBarDB.barColor

-- ===== Automatic honor cap & rank milestones =====
--  Convert Rank + Rank Progress to Contribution Points (CP) using per-rank floors and ceilings.
--  Simulate CP gained across rank "buckets" (the CP span between adjacent rank floors).
--  The first bucket is capped by remaining space in the current rank band; some ranks use fixed first bucket awards.
--  Some multirank hops add a small bonus CP; variance tables provide best/expected/worst near thresholds.
...
function HB_Rank:CalculateCPGain(rank, objectiveRank, predictedCP, currentCP)
  ...
  for key = rank + 1, objectiveRank do
    ...
    if key == rank + 1 then
      -- Some ranks use fixed first-bucket awards (guardrails to match observed in-game behavior).
      if rank == 9 then
        gainedCP = 3000
      elseif rank == 11 then
        gainedCP = 2500
      ...
      -- Certain multi-rank hop patterns add a small bonus CP (exception rule).
...
local function UpdateAutoCapFromRank()
  ...
  -- If auto is disabled it will use the manually set cap.
  ...
  -- Milestones for ticks/tooltips are filtered to:
  --   * honorNeed above current honor
  --   * rank at/under the objective rank
  --   * skip "+1" outcomes (special-case in the model)
  --   * stop after maxObtainableRank
  ...
  -- If filter yields nothing (at or above objective)
  -- then fall back to "max honorNeed" cap using all options.
  if #filtered == 0 then
    local maxHonor = 0
...
local Frame = CreateFrame("Frame", "HonorBarFrame", UIParent)
...
local function HB_PlaySound(path)
  ...
  -- Use the Master channel so it is heard even if some channels are muted
...
local function HB_CheckHonorSounds(oldHonor, newHonor)
  ...
  -- First time seeing honor for this session, exit with no sound.
  if prev == nil then
    HonorBarDB.lastHonorForSounds = newHonor
    return
  end

  -- If honor didn't increase since last refresh, do nothing.
  if not oldHonor or newHonor <= oldHonor then
    return
  end

  -- Use the last honor value as starting point for checks.
  local old = prev

  -- If we haven't exceeded the baseline, don't play sounds.
   if newHonor <= old then
    return
  end
  ...
  -- 1) Weekly honor cap reached then play cap sound when crossing to cap.
  if cap and cap > 0 then
    if old < cap and newHonor >= cap then
      HB_PlaySound(capSoundPath)
    end
  end

  -- 2) If milestone reached, only in auto mode with milestone data.
  if HonorBarDB.autoCapFromRank and rankMilestones and type(rankMilestones) == "table" and #rankMilestones > 0 then
    local currentHonor = newHonor
    for _, m in ipairs(rankMilestones) do
      local mh = m and m.honor
      if mh and mh > 0 and old < mh and currentHonor >= mh then
        HB_PlaySound(milestoneSoundPath)
        break  -- Only play once per refresh, even if multiple milestones were crossed.
      end
    end
  end

  -- Update baseline after checks to higher honor value.
  HonorBarDB.lastHonorForSounds = newHonor
end
...
local function CreateOrUpdateMilestoneTicks()
  ...
  -- Only draw milestones in auto mode when there is milestone data
...
local function HP_OpenAdjustDialog()
  ...
  if not HP_AdjustDlg then
    HP_AdjustDlg = CreateFrame("Frame", "HonorBarAdjustDlg", UIParent, "BackdropTemplate")
    if UISpecialFrames then table.insert(UISpecialFrames, "HonorBarAdjustDlg") end
    -- Slightly taller dialog to prevent controls from feeling cramped.
    HP_AdjustDlg:SetSize(380, 360)
...
hideTextCB:SetChecked(HonorBarDB.hideBarText or false)
    hideTextCB:SetScript("OnClick", function(self)
      HonorBarDB.hideBarText = self:GetChecked() and true or false
...
onlyNumbersCB:SetChecked(HonorBarDB.onlyNumbers or false)
onlyNumbersCB:SetScript("OnClick", function(self)
  HonorBarDB.onlyNumbers = self:GetChecked() and true or false
  if UpdateBar then UpdateBar() end
end)
...
labelsBottomCB:SetChecked(HonorBarDB.milestoneLabelsBottom or false)
labelsBottomCB:SetScript("OnClick", function(self)
  HonorBarDB.milestoneLabelsBottom = self:GetChecked() and true or false
  CreateOrUpdateMilestoneTicks()
end)
...
-- Hide Bar
    ...
    hideBarCB:SetChecked(HonorBarDB.hiddenBar or false)
    hideBarCB:SetScript("OnClick", function(self)
      HonorBarDB.hiddenBar = self:GetChecked() and true or false
      ApplyVisibility()
    end)
    ...
    hideTicksCB:SetChecked(HonorBarDB.hideTicks or false)
    hideTicksCB:SetScript("OnClick", function(self)
      HonorBarDB.hideTicks = self:GetChecked() and true or false
      CreateOrUpdateTicks()
    end)
...
-- Enable !honor
    ...
    honorReplyCB:SetChecked(HonorBarDB.replyEnabled ~= false)
    honorReplyCB:SetScript("OnClick", function(self)
      HonorBarDB.replyEnabled = self:GetChecked() and true or false
    end)
...
-- Anchor checkboxes
    ...
    local function MakeColorSwatch(name, label, x, y, getter, setter)
      ...
      btn:SetScript("OnClick", function(self)
        ...
        -- May call swatchFunc instead of func.
    ...
    local function setW(v) HP_SetSizeAndSave(v, HonorBarDB.height or (Frame and Frame:GetHeight() or 22)) end
    local function getH() return HonorBarDB.height or (Frame and Frame:GetHeight() or 22) end
    local function setH(v) HP_SetSizeAndSave(HonorBarDB.width or (Frame and Frame:GetWidth() or 300), v) end
    
    local function getRefresh()
      return HonorBarDB.statsRefresh or 1.0
    end
    local function setRefresh(v)
      ...
      HonorBarDB.statsRefresh = tonumber(string.format("%.1f", v))
      if autoOn then
    ...
    MakeColorSwatch("HP_Adjust_BarColor",       "Bar Fill Color",        -120, -230, getBarColor,     setBarColor)
    MakeColorSwatch("HP_Adjust_BarBgColor",    "Bar Background",     -40,  -230, getBarBgColor,  setBarBgColor)
    MakeColorSwatch("HP_Adjust_TickColor",     "Tick Color",          40,  -230, getTickRGB,      setTickRGB)
    MakeColorSwatch("HP_Adjust_MilestoneColor","Milestone Color",    120,  -230, getMilestoneRGB, setMilestoneRGB)
    ...
    closeBtn:SetPoint("BOTTOM", HP_AdjustDlg, "BOTTOM", 0, 16)
...
local function HB_UpdateCurrentGameFromAPIs(deltaHonor)
  ...
  if weeklyHonor ~= nil then
    if matchHonorBaseline == nil then
      -- First update grab baseline, dont count previous honor
...
local function HB_StartNewMatch()
  ...
  -- Never reset match stats unless actually in BG
  ...
  -- Swap current game to last game
  HonorBarDB.lastGameHonor = HonorBarDB.currentGameHonor or HonorBarDB.lastGameHonor or 0
  HonorBarDB.lastGameHK    = HonorBarDB.currentGameHK or HonorBarDB.lastGameHK or 0
  HonorBarDB.lastGameKB    = HonorBarDB.currentGameKB or HonorBarDB.lastGameKB or 0
  HonorBarDB.lastGameTime  = HonorBarDB.currentGameTime or HonorBarDB.lastGameTime or 0
...
local function HB_CheckBG()
  ...
  if inInstance and type(inInstance) == "boolean" then
    -- Classic IsInInstance() returns (inInstance, instanceType)
  ...
  if nowInBG then
    ...
    -- Finalize current match duration, clear tracking
    if matchActive and matchStartTime and HonorBarDB then
      local now = GetTime and GetTime() or nil
      if now and now > matchStartTime then
        local elapsed = math.floor(now - matchStartTime)
        if elapsed > 0 then
          HonorBarDB.currentGameTime = (HonorBarDB.currentGameTime or 0) + elapsed
        end
      end
    end
    matchStartTime = nil
...
local function RefreshHonor(verbose)
  ...
  if type(new) == "number" then
    ...
    -- Sound notifications for milestones and weekly honor cap
  ...
  if HonorBarDB.autoCapFromRank and (weeklyHonor or 0) ~= oldWeekly then
    UpdateAutoCapFromRank()
  end
...
local function StartAuto()
  ...
  local interval = 1.0
  if HonorBarDB and HonorBarDB.statsRefresh then
    interval = HonorBarDB.statsRefresh
  end
...
local function UpdateStatsFrame()
  ...
  -- If bar text is hidden, shows the same bar text string here between Honor/hour and Remaining.
  if HonorBarDB and HonorBarDB.hideBarText then
    local cap = (honorCap and honorCap > 0) and honorCap or 1
    local pct = math.min(((weeklyHonor or 0) / cap) * 100, 100)
    local barLine
    if HonorBarDB.showText then
      if HonorBarDB.onlyNumbers then
  ...
  do
    local parts = {}
    -- Only show milestones in auto mode
  ...
  if HonorBarDB then
    ...
    -- If a match active, show a live timer
    ...
    local lastTimeStr = secsToHMS(lastTime)
...
local function UpdateGameTimersOnly()
  ...
  -- Replace only the Time portion of the Current Game line, preserving Honor/HK/KB.
...
local function UpdateTooltip()
  ...
  local sessionHK = GetSessionHK()
  local etaSecs = (hph > 0) and (remaining / hph * 3600) or nil
  local detachStats = HonorBarDB and HonorBarDB.detachStats
  ...
  -- Show honor remaining to each upcoming milestone minus the last
  do
    local parts = {}
    -- Only show milestones in auto mode
    if HonorBarDB and HonorBarDB.autoCapFromRank and rankMilestones and type(rankMilestones) == "table" and #rankMilestones > 1 then
  ...
  if HonorBarDB then
    ...
    -- Time is handled by UpdateTooltipGameTimersOnly
    local before = GameTooltip:NumLines()
    GameTooltip:AddLine(string.format("Current Game: Honor %s HKs %d KBs %d Time --", fmtnum(curHonor), curHK, curKB), 1,1,1, true)
    tooltipCurrentGameLineIndex = before + 1

    before = GameTooltip:NumLines()
    GameTooltip:AddLine(string.format("Last Game: Honor %s HKs %d KBs %d Time --", fmtnum(lastHonor), lastHK, lastKB), 1,1,1, true)
  end
...
Frame:SetScript("OnUpdate", function(self, elapsed)
  ...
  local refresh = 1.0
  if HonorBarDB and HonorBarDB.statsRefresh then
    refresh = HonorBarDB.statsRefresh
  end
  if refresh < 0.2 then
    refresh = 0.2
...
local function HP_SuggestCommand(input)
  local cmds = { "help","config","debug","mem","resetsounds","cap","auto","resetpos","show","hide","toggle" }
...
function RootSlashHandler(msg)
  ...
  if cmd == "" or cmd == "help" then
    if HP_ShowHelpPopup then HP_ShowHelpPopup() end
  ...
  elseif cmd == "resetsounds" then
    ...
    HP_Print("sound baseline reset to current weekly honor (" .. tostring(cur) .. ").")
  elseif cmd == "cap" then
    if HonorBarDB.autoCapFromRank then
  ...
  elseif cmd == "auto" then
    if rest == "on" then
      autoOn = true; HonorBarDB.autoOn = true; StartAuto(); HP_Print("auto ON")
    elseif rest == "off" then
      autoOn = false; HonorBarDB.autoOn = false; StopAuto(); HP_Print("auto OFF")
    else
...
-- =================== Honor Bar: Simple !honor auto-reply ===================
...
-- ============================================================================
--  Optional Modules (inlined): Help Dialog and Pull Timer
--  Refactor: merged HonorHelp.lua and HonorPull.lua for simpler packaging.
-- ============================================================================
...
local function BuildHelpText()
  return table.concat({
    hdr("Slash commands:"),
    "  " .. cmd("/honor") .. " or " .. cmd("/hb") .. "  - open this help window",
    "  " .. cmd("/honor config") .. "        - open the Bar Config window",
    "  " .. cmd("/honor debug") .. "         - force honor refresh & print debug info",
    "  " .. cmd("/honor mem") .. "           - print addon memory usage",
    "  " .. cmd("/honor cap <number>") .. "  - set weekly honor cap (e.g. /honor cap 750000)",
    "  " .. cmd("/honor auto on|off") .. "   - toggle 1s auto-refresh",
    "  " .. cmd("/honor resetsounds") .. "    - reset sound baseline to current weekly honor",
    "  " .. cmd("/honor resetpos") .. "      - reset bar position to screen center",
    "  " .. cmd("/honor show|hide|toggle") .. " - show, hide, or toggle the bar",
    "",
    hdr("!honor auto-reply:"),
    "  " .. cmd("/hbreply on|off") .. "      - enable or disable replying to '!honor'",
    "  When enabled, typing " .. cmd("!honor") .. " in chat replies with your weekly honor status.",
    "",
    hdr("Pull timer:"),
    "  " .. cmd("/honorpull [sec]") .. " or " .. cmd("/hbpull [sec]") .. " - start pull countdown (default 10s, range 2-30)",
    "  " .. cmd("/cpull") .. "               - cancel the active pull timer",
    "",
    hdr("Config checkboxes (in /honor config):"),
    "  " .. cmd("Hide Bar Text") .. "        - hide the numbers/text on the bar",
    "  " .. cmd("Only Numbers") .. "         - show compact numeric text only",
    "  " .. cmd("Bar labels bottom") .. "    - show milestone labels below the bar instead of above",
    "  " .. cmd("Hide Bar") .. "             - completely hide the bar frame",
    "  " .. cmd("Hide Ticks") .. "           - hide tick marks on the bar",
    "  " .. cmd("Enable !honor") .. "        - allow the addon to reply to '!honor'",
    "  " .. cmd("Auto (Honor Cap)") .. "     - auto-set weekly cap from your current rank",
    "  " .. cmd("Detach Stats") .. "          - show session stats in a movable transparent frame",
    "",
    hdr("Config sliders / numeric inputs:"),
    "  " .. cmd("Width") .. "                - bar width in pixels",
    "  " .. cmd("Height") .. "               - bar height in pixels",
    "  " .. cmd("Stats Refresh (s)") .. "    - seconds between honor/stats updates",
    "  " .. cmd("Tick Width") .. "           - thickness of tick marks",
    "  " .. cmd("Tick Amount") .. "          - number of tick marks on the bar (0-60)",
    "  " .. cmd("Honor Cap") .. "            - manual weekly honor cap (disabled if Auto is on)",
    "",
    hdr("Config color swatches:"),
    "  " .. cmd("Bar Fill Color") .. "  - color of the bar fill",
    "  " .. cmd("Bar Background") .. "  - background color behind the bar",
    "  " .. cmd("Tick Color") .. "           - color for normal tick marks",
    "  " .. cmd("Milestone Color") .. "      - color for milestone tick marks",
  }, "\n")
end
