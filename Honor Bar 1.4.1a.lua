  ...
  if not HP_AdjustDlg then
    ...
    helpBtn:SetScript("OnClick", function()
      ...
      elseif type(PrintHelp) == "function" then
        ...
        -- Checkboxes (table-driven)
    local function MakeCheckbox(name, text, x, y, getFn, setFn, fontObj)
      local cb = CreateFrame("CheckButton", name, HP_AdjustDlg, "UICheckButtonTemplate")
      cb:SetSize(18, 18)
      cb:SetScale(1.0)
      cb:SetPoint("TOP", HP_AdjustDlg, "TOP", x, y)

      local label = _G[name.."Text"] or cb.Text
      if label then
        label:SetText(text)
        label:ClearAllPoints()
        label:SetPoint("BOTTOM", cb, "TOP", 0, 0)
    ...
    local CHECKS = {
      { "HP_Adjust_HideText", "Hide Bar Text", 0, -270,
        function() return HonorBarDB.hideBarText or false end,
        function(v) HonorBarDB.hideBarText = v; if UpdateBar then UpdateBar() end end },

      { "HP_Adjust_HideBar", "Hide Bar", -100, -270,
        function() return HonorBarDB.hiddenBar or false end,
        function(v) HonorBarDB.hiddenBar = v; ApplyVisibility() end },

      { "HP_Adjust_HideTicks", "Hide Ticks", 100, -270,
        function() return HonorBarDB.hideTicks or false end,
        function(v) HonorBarDB.hideTicks = v; CreateOrUpdateTicks() end },

      { "HP_Adjust_OnlyNumbers", "Only Numbers", -100, -302,
        function() return HonorBarDB.onlyNumbers or false end,
        function(v) HonorBarDB.onlyNumbers = v; if UpdateBar then UpdateBar() end end },

      { "HP_Adjust_LabelsBottom", "Bar Labels Bottom", 0, -302,
        function() return HonorBarDB.milestoneLabelsBottom or false end,
        function(v) HonorBarDB.milestoneLabelsBottom = v; CreateOrUpdateMilestoneTicks() end },

      { "HP_Adjust_EnableHonor", "Enable !honor", 100, -302,
        function() return HonorBarDB.replyEnabled ~= false end,
        function(v) HonorBarDB.replyEnabled = v end,
        GameFontNormalSmall },
    }

    for _, c in ipairs(CHECKS) do
      MakeCheckbox(c[1], c[2], c[3], c[4], c[5], c[6], c[7])
    end
...
-- Anchor checkboxes
    ...
    MakeColorSwatch("HP_Adjust_BarColor",       "Fill",        -120, -230, getBarColor,     setBarColor)
    MakeColorSwatch("HP_Adjust_BarBgColor",    "Background",     -40,  -230, getBarBgColor,  setBarBgColor)
    MakeColorSwatch("HP_Adjust_TickColor",     "Tick",          40,  -230, getTickRGB,      setTickRGB)
    MakeColorSwatch("HP_Adjust_MilestoneColor","Milestone",    120,  -230, getMilestoneRGB, setMilestoneRGB)
    ...
    if showText and not hideBarText then
      ...
      if HonorBarDB.autoCapFromRank then
        ...
        elseif goalReached then
          -- Final-week check: only treat it as the final Rank 13 -> 14 push if
          --   the weekly cap matches the known final cap (~418750), AND
          --   the player is Rank 13 at ~67.84% progress.
...
local function UpdateGameTimersOnly()
  ...
  -- Replace only the Time portion of the Current Game preserve Honor/HK/KB.
...
local function UpdateTooltip()
  ...
  -- When session stats are detached into their own frame, the bar tooltip should
  -- only show the interaction hints (no session stats lines).
...
local function PrintHelp()
  ...
  print(" /honor resetsounds      - reset sound baseline to current weekly honor")
...
function RootSlashHandler(msg)
  ...
  if cmd == "" or cmd == "help" then
    if type(HP_ShowHelpPopup) == "function" then
      HP_ShowHelpPopup()
    else
      PrintHelp()
    end
  ...
  elseif cmd == "resetsounds" then
    if not HonorBarDB then
      HP_Print("HonorBar: no saved variables yet; try again after the bar loads.")
    else
      local cur = (GetWeeklyHonor and GetWeeklyHonor()) or 0
      HonorBarDB.lastHonorForSounds = cur
      HP_Print("HonorBar: sound baseline reset to current weekly honor (".. tostring(cur) ..").")
    end
...
local function BuildHelpText()
  return table.concat({
    ...
    "  " .. cmd("Fill") .. "              - color of the bar fill",
    "  " .. cmd("Background") .. "        - background color behind the bar",
    "  " .. cmd("Tick") .. "              - color for normal tick marks",
    "  " .. cmd("Milestone") .. "         - color for milestone tick marks",
  }, "\n")
end
...
local function EnsureHelpFrame()
  ...
  -- Anchor the scrollframe inside the main frame
...
-- Fallback if LE_PARTY_CATEGORY_INSTANCE is undefined
