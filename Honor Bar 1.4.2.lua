...
local function ApplyDefaults()
  ...
  -- Defaults where false/nil = not set
  local defaultsNot = {
...
-- Forward declarations for layering
local Frame
local TickOverlay
local TextOverlay
...
local function HB_CheckHonorSounds(oldHonor, newHonor)
  ...
  -- If sounds are disabled, still maintain a moving baseline to avoid stale comparisons.
  ...
  -- Start baseline on stable honor value this session
...
local function HB_OpenAdjustDialog()
  ...
  if not HB_AdjustDlg then
    ...
    local function MakeCheckbox(name, text, x, y, getFn, setFn, fontObj)
      local cb = CreateFrame("CheckButton", name, HB_AdjustDlg, "UICheckButtonTemplate")
      cb:SetSize(18, 18)
      cb:SetScale(1.0)
      cb:SetPoint("TOP", HB_AdjustDlg, "TOP", x, y)
    ...
    local CHECKS = {
      { "HB_Adjust_HideText", "Hide Bar Text", -50, -270,
      ...
      { "HB_Adjust_LabelsBottom", "Bar Labels Bottom", -50, -302,
      ...
      { "HB_Adjust_EnableHonor", "Enable !honor", 50, -302,
        function() return HonorBarDB.replyEnabled ~= false end,
        function(v) HonorBarDB.replyEnabled = v end,
        GameFontNormalSmall },
    
      { "HB_Adjust_Sound", "Sound", 140, -270,
        function() return HonorBarDB.soundEnabled ~= false end,
        function(v) HonorBarDB.soundEnabled = v end },

      { "HB_Adjust_SendBack", "Send Back", 140, -302,
        function() return HonorBarDB.sendBack or false end,
        function(v) HonorBarDB.sendBack = v; if HB_ApplyBarStrata then HB_ApplyBarStrata() end end },
}
...
-- Anchor checkboxes
    ...
    local widthSlider  = MakeSlider("HB_Adjust_Width",  "Width",        100, 1000, 10, getW,  setW,  -90, -46)
    ...
    MakeColorSwatch("HB_Adjust_BarColor",       "Fill",        -120, -230, getBarColor,     setBarColor)
    MakeColorSwatch("HB_Adjust_BarBgColor",    "Background",     -40,  -230, getBarBgColor,  setBarBgColor)
    MakeColorSwatch("HB_Adjust_TickColor",     "Tick",          40,  -230, getTickRGB,      setTickRGB)
    MakeColorSwatch("HB_Adjust_MilestoneColor","Milestone",    120,  -230, getMilestoneRGB, setMilestoneRGB)
...
-- State
local weeklyHonor = 0
-- Stable honor sync state (prevents transient 0/drops from poisoning UI, sounds, and BG baselines)
local HB_HonorSynced = false

-- Hold weekly honor from last known value
...
local function HB_StartNewMatch()
  ...
  -- Honor baseline:
  -- - If weekly honor is already synced, capture it immediately on BG entry.
  -- - Otherwise defer until the first synced refresh tick inside the BG.
...
local function HB_CheckBG()
  ...
  if nowInBG then
    ...
    -- Leaving BG stop updating stats, but no 0 and Current Game remains visible until the next BG entry
...
-- Stable weekly honor filtering: transient 0 reads during login/zone load, brief honor drops before APIs settle, stableValue, isProvisional, isSynced
...
local function HB_GetStableWeeklyHonor(rawVal, fromAPI)
  ...
  -- If we didn't get read, then not mark synced.
  if not fromAPI then
  ...
  -- 0 reads are common during login/zone load; persist before syncing to 0.
  ...
  -- Any non-zero value clears the "pending zero" timer.
  HB_WeeklyPendingZeroStart = nil

  -- First decrease: show previous value briefly until confirmed.
...
local function RefreshHonor(verbose)
  ...
  -- Consider an update "synced" only when we have a concrete API read.
  ...
  -- Persist last known stable value only when synced (i.e., derived from real API reads)
...
local function BuildHelpText()
  return table.concat({
    ...
    "  " .. cmd("Send Back") .. "            - sends the bar to bottom layer so it doesn't block other objects from view",
    "  " .. cmd("Sound") .. "                - enable/disable milestone/honor cap audible notifications",
    "",
